...
public:
    ...
    void init(Severity min_severity, bool async, bool enable_console, const std::string &filename, size_t max_file_size, size_t max_files,
              bool flush_every_time)
    {
        static appender appender(filename, async, enable_console, max_file_size, max_files, flush_every_time);
        async_          = async;
        appender_       = &appender;
    ...
    void add_appender(std::function<void(std::string_view)> fn) { appenders_.emplace_back(std::move(fn)); }
...
template <size_t Id = 0>
inline void init_log(Severity min_severity, const std::string &filename = "", bool async = true, bool enable_console = true,
                     size_t max_file_size = 0, size_t max_files = 0, bool flush_every_time = false)
{
    logger<Id>::instance().init(min_severity, async, enable_console, filename, max_file_size, max_files, flush_every_time);
}
...
#define ELOG_IMPL(severity, Id, ...)                               \
    ...
    {                                                              \
        ...
        easylog::logger<Id>::instance() +=                         \
            easylog::record_t(std::chrono::system_clock::now(), severity, GET_STRING(__FILE__, __LINE__)).ref()

#define ELOG(severity, ...) ELOG_IMPL(easylog::Severity::severity, __VA_ARGS__, 0)
...
#define ELOGV_IMPL(severity, Id, fmt, ...)                                                                                           \
    ...
    {                                                                                                                                \
        easylog::logger<Id>::instance() +=                                                                                           \
            easylog::record_t(std::chrono::system_clock::now(), severity, GET_STRING(__FILE__, __LINE__)).sprintf(fmt, __VA_ARGS__); \
...
#define ELOGV(severity, ...) ELOGV_IMPL(easylog::Severity::severity, 0, __VA_ARGS__, "\n")

#define MELOGV(severity, Id, ...) ELOGV_IMPL(easylog::Severity::severity, Id, __VA_ARGS__, "\n")
