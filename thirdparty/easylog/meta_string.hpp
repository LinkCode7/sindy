...
struct meta_string
{
    ...
    constexpr meta_string(std::span<const char, Ns>... data) noexcept : elements_{}
    {
    ...
    constexpr char& operator[](std::size_t index) noexcept { return elements_[index]; }

    constexpr const char& operator[](std::size_t index) const noexcept { return elements_[index]; }

    constexpr operator std::string_view() const noexcept { return std::string_view{elements_.data(), size()}; }
    ...
    constexpr size_t rfind(char c) const noexcept { return std::string_view(*this).rfind(c); }

    constexpr size_t find(char c) const noexcept { return std::string_view(*this).find(c); }
};
...
constexpr auto operator<=>(const meta_string<M>& left, const meta_string<N>& right) noexcept
{
...
template <std::size_t M, std::size_t N>
constexpr bool operator==(const meta_string<M>& left, const meta_string<N>& right) noexcept
{
    return static_cast<std::string_view>(left) == static_cast<std::string_view>(right);
}

template <std::size_t M, std::size_t N>
constexpr bool operator==(const meta_string<M>& left, const char (&right)[N]) noexcept
{
    return static_cast<std::string_view>(left) == static_cast<std::string_view>(meta_string{right});
}
...
template <std::size_t M, std::size_t N>
constexpr auto operator+(const meta_string<M>& left, const char (&right)[N]) noexcept
{
    meta_string<M + N - 1> s;
...
template <std::size_t M, std::size_t N>
constexpr auto operator+(const char (&left)[M], const meta_string<N>& right) noexcept
{
    meta_string<M + N - 1> s;
...
struct split_of
{
    static constexpr auto value = [] {
        ...
        constexpr auto             group_count = std::count_if(S.begin(), S.end(), [](char c) { return Delim.contains(c); }) + 1;
        std::array<std::string_view, group_count> result{};

        auto iter = result.begin();

        for (std::size_t start_index = 0, end_index = view.find_first_of(Delim);;
             start_index = end_index + 1, end_index = view.find_first_of(Delim, start_index))
        {
...
struct split
{
    ...
    static constexpr auto             value = [] {
        ...
        for (std::size_t start_index = 0, end_index = view.find(Delim);;
             start_index = end_index + Delim.size(), end_index = view.find(Delim, start_index))
        {
            *(iter++) = view.substr(start_index, end_index - start_index);
...
struct remove_char
{
    static constexpr auto value = [] {
        ...
        constexpr auto metadata = [] {
            ...
            return removal_metadata{.result{std::move(result)}, .actual_size{static_cast<std::size_t>(removal_end - result.begin())}};
        }();
...
struct remove
{
    ...
    static constexpr auto value  = [] {
        ...
        {
            return meta_string{std::span<const char, groups[Is].size()>{groups[Is].data(), groups[Is].size()}...};
        }
