...
inline char *get_time_str(const auto &now)
{
    ...
    auto                                mill_sec = std::chrono::duration_cast<std::chrono::milliseconds>(d - s).count();
    int                                 size     = 23;
    if (last_sec_ == s)
    {
...
public:
    appender() = default;
    appender(const std::string &filename, bool async, bool enable_console, size_t max_file_size, size_t max_files, bool flush_every_time)
        : has_init_(true), flush_every_time_(flush_every_time), enable_console_(enable_console), max_file_size_(max_file_size)
    {
        filename_  = filename;
    ...
    void start_thread()
    {
        write_thd_ = std::thread([this] {
            while (!stop_)
            {
                if (max_files_ > 0 && file_size_ > max_file_size_ && static_cast<size_t>(-1) != file_size_)
                {
                    roll_log_files();
                }
                ...
                {
                    enable_console_ ? write_record<false, true>(record) : write_record<false, false>(record);
                }
