...
{
template <class T>
constexpr inline bool c_array_v = std::is_array_v<std::remove_cvref_t<T>> && std::extent_v<std::remove_cvref_t<T>> > 0;
...
public:
    ...
    record_t(auto tm_point, Severity severity, std::string_view str)
        : tm_point_(tm_point), severity_(severity), tid_(_get_tid()), file_str_(str)
    {
        ss_.reserve(64);
    }
