...
struct default_float_traits
{
    ...
    using format = std::conditional_t<detail::physical_bits<T> == 32, ieee754_binary32, ieee754_binary64>;
    ...
    using carrier_uint = std::conditional_t<detail::physical_bits<T> == 32, std::uint32_t, std::uint64_t>;
    ...
    // sign bit.
    static constexpr carrier_uint remove_sign_bit_and_shift(carrier_uint u) noexcept { return carrier_uint(carrier_uint(u) << 1); }
    ...
    static constexpr carrier_uint binary_significand(carrier_uint significand_bits, unsigned int exponent_bits) noexcept
    {
        if (exponent_bits == 0)
        {
    ...
    static constexpr bool has_all_zero_significand_bits(carrier_uint u) noexcept { return (u << 1) == 0; }
...
struct float_bits
{
    ...
    // The result must be aligned to the LSB so that there is no additional zero
    // paddings on the right. This function does not do bias adjustment.
    constexpr unsigned int extract_exponent_bits() const noexcept { return traits_type::extract_exponent_bits(u); }

    // Extract significand bits from a bit pattern.
    // The result must be aligned to the LSB so that there is no additional zero
    // paddings on the right. The result does not contain the implicit bit.
    constexpr carrier_uint extract_significand_bits() const noexcept { return traits_type::extract_significand_bits(u); }
    ...
    // Obtain the actual value of the binary exponent from the extracted exponent
    // bits.
    static constexpr int binary_exponent(unsigned int exponent_bits) noexcept { return traits_type::binary_exponent(exponent_bits); }
    constexpr int        binary_exponent() const noexcept { return binary_exponent(extract_exponent_bits()); }
    ...
    constexpr bool is_finite(unsigned int exponent_bits) const noexcept { return traits_type::is_finite(exponent_bits); }
...
struct signed_significand_bits
{
    ...
    // Shift the obtained signed significand bits to the left by 1 to remove the
    // sign bit.
    constexpr carrier_uint remove_sign_bit_and_shift() const noexcept { return traits_type::remove_sign_bit_and_shift(u); }

    constexpr bool is_positive() const noexcept { return traits_type::is_positive(u); }
    constexpr bool is_negative() const noexcept { return traits_type::is_negative(u); }
    constexpr bool has_all_zero_significand_bits() const noexcept { return traits_type::has_all_zero_significand_bits(u); }
    constexpr bool has_even_significand_bits() const noexcept { return traits_type::has_even_significand_bits(u); }
};
...
JKJ_SAFEBUFFERS inline uint128 umul128(std::uint64_t x, std::uint64_t y) noexcept
{
    ...
    return {ac + (intermediate >> 32) + (ad >> 32) + (bc >> 32), (intermediate << 32) + std::uint32_t(bd)};
#endif
}
...
{
    using namespace log;
    return compute<multiply(315653), subtract(0), shift(20), min_exponent(floor_log10_pow2_min_exponent),
...
{
    using namespace log;
    return compute<multiply(1741647), subtract(0), shift(19), min_exponent(floor_log2_pow10_min_exponent),
...
{
    using namespace log;
    return compute<multiply(631305), subtract(261663), shift(21), min_exponent(floor_log10_pow2_minus_log10_4_over_3_min_exponent),
...
{
    using namespace log;
    return compute<multiply(225799), subtract(0), shift(19), min_exponent(floor_log5_pow2_min_exponent),
...
{
    using namespace log;
    return compute<multiply(451597), subtract(715764), shift(20), min_exponent(floor_log5_pow2_minus_log5_3_min_exponent),
...
constexpr UInt divide_by_pow10(UInt n) noexcept
{
    ...
    if constexpr (std::is_same_v<UInt, std::uint64_t> && N == 3 && n_max <= std::uint64_t(15534100272597517998ull))
    {
...
struct compressed_cache_detail
{
    ...
    static constexpr std::size_t compressed_table_size =
        (cache_holder<ieee754_binary64>::max_k - cache_holder<ieee754_binary64>::min_k + compression_ratio) / compression_ratio;
...
struct nearest_to_even : base
{
    ...
    template <class SignedSignificandBits, class Func>
    JKJ_FORCEINLINE static constexpr auto invoke_normal_interval_case(SignedSignificandBits s, Func &&f) noexcept
    {
        return f(s.has_even_significand_bits());
    }
    template <class SignedSignificandBits, class Func>
    JKJ_FORCEINLINE static constexpr auto invoke_shorter_interval_case(SignedSignificandBits, Func &&f) noexcept
    {
        return f();
    }
};
...
struct nearest_toward_plus_infinity : base
{
    ...
    template <class SignedSignificandBits, class Func>
    JKJ_FORCEINLINE static constexpr auto invoke_shorter_interval_case(SignedSignificandBits s, Func &&f) noexcept
    {
        return f(!s.is_negative());
    }
};
...
struct nearest_toward_zero : base
{
    ...
    template <class SignedSignificandBits, class Func>
    JKJ_FORCEINLINE static constexpr auto invoke_normal_interval_case(SignedSignificandBits, Func &&f) noexcept
    {
        return f();
    }
    template <class SignedSignificandBits, class Func>
...
struct nearest_to_even_static_boundary : base
{
    ...
    template <class SignedSignificandBits, class Func>
    JKJ_FORCEINLINE static auto delegate(SignedSignificandBits s, Func &&f) noexcept
    {
...
struct compact : base
{
    ...
    static constexpr typename cache_holder<FloatFormat>::cache_entry_type get_cache(int k) noexcept
    {
        ...
        if constexpr (std::is_same_v<FloatFormat, ieee754_binary64>)
        {
            ...
            auto const cache_index = int(std::uint32_t(k - cache_holder<FloatFormat>::min_k) / compressed_cache_detail::compression_ratio);
            auto const kb          = cache_index * compressed_cache_detail::compression_ratio + cache_holder<FloatFormat>::min_k;
            ...
            else
            {
                ...
                recovered_cache =
                    wuint::uint128{(recovered_cache.low() >> alpha) | high_to_middle, ((middle_low.low() >> alpha) | middle_to_low)};
...
struct impl : private FloatTraits, private FloatTraits::format
{
    ...
    static constexpr int min_k = [] {
        constexpr auto a = -log::floor_log10_pow2_minus_log10_4_over_3(int(max_exponent - significand_bits));
        constexpr auto b = -log::floor_log10_pow2(int(max_exponent - significand_bits)) + kappa;
        return a < b ? a : b;
    }();
    ...
    static constexpr int max_k = [] {
        ...
        constexpr auto a = -log::floor_log10_pow2_minus_log10_4_over_3(int(min_exponent - significand_bits /*+ 1*/));
        constexpr auto b = -log::floor_log10_pow2(int(min_exponent - significand_bits)) + kappa;
        return a > b ? a : b;
    }();
    ...
    template <class ReturnType, class IntervalType, class TrailingZeroPolicy, class BinaryToDecimalRoundingPolicy, class CachePolicy,
              class... AdditionalArgs>
    JKJ_SAFEBUFFERS static ReturnType compute_nearest_normal(carrier_uint const two_fc, int const exponent,
                                                             AdditionalArgs... additional_args) noexcept
    {
        ...
        else
        {
            ...
            auto const [xi_parity, x_is_integer] = compute_mul_parity(two_fc - 1, cache, beta);
    ...
    small_divisor_case_label:
        ...
        else
        {
            ...
            if (divisible_by_small_divisor)
            {
                ...
                // is an even number.
                auto const [yi_parity, is_y_integer] = compute_mul_parity(two_fc, cache, beta);
    ...
    template <class ReturnType, class IntervalType, class TrailingZeroPolicy, class BinaryToDecimalRoundingPolicy, class CachePolicy,
              class... AdditionalArgs>
    JKJ_SAFEBUFFERS static ReturnType compute_nearest_shorter(int const exponent, AdditionalArgs... additional_args) noexcept
    {
        ReturnType   ret_value;
        IntervalType interval_type{additional_args...};
        ...
        if (BinaryToDecimalRoundingPolicy::prefer_round_down(ret_value) && exponent >= shorter_interval_tie_lower_threshold &&
    ...
    JKJ_SAFEBUFFERS static ReturnType compute_left_closed_directed(carrier_uint const two_fc, int exponent) noexcept
    {
        //////////////////////////////////////////////////////////////////////
        // Step 1: Schubfach multiplier calculation
        //////////////////////////////////////////////////////////////////////
        ...
        else if (r == deltai)
        {
            ...
            // 10^-17).
            auto const [zi_parity, is_z_integer] = compute_mul_parity(two_fc + 2, cache, beta);
    ...
    template <class ReturnType, class TrailingZeroPolicy, class CachePolicy>
    JKJ_SAFEBUFFERS static ReturnType compute_right_closed_directed(carrier_uint const two_fc, int const exponent,
                                                                    bool shorter_interval) noexcept
    {
        ...
        // Compute zi and deltai.
        // 10^kappa <= deltai < 10^(kappa + 1)
        auto const         deltai = shorter_interval ? compute_delta(cache, beta - 1) : compute_delta(cache, beta);
    ...
    static constexpr std::uint32_t compute_delta(cache_entry_type const &cache, int beta) noexcept
    {
        if constexpr (std::is_same_v<format, ieee754_binary32>)
        {
...
constexpr auto make_policy_holder(BaseDefaultPairList, Policies... policies)
{
    ...
    using policy_pair_list = decltype(make_policy_holder_impl(BaseDefaultPairList{}, found_policy_pair_list<false>{}, policies...));

    static_assert(!policy_pair_list::repeated, "jkj::dragonbox: each policy should be specified at most once");
...
JKJ_FORCEINLINE JKJ_SAFEBUFFERS auto to_decimal(signed_significand_bits<Float, FloatTraits> signed_significand_bits,
                                                unsigned int                                exponent_bits, Policies... policies) noexcept
{
    ...
    using return_type =
        decimal_fp<typename FloatTraits::carrier_uint, policy_holder::return_has_sign, policy_holder::report_trailing_zeros>;

    return_type ret = policy_holder::delegate(signed_significand_bits, [exponent_bits,
                                                                        signed_significand_bits](auto interval_type_provider) {
        using format       = typename FloatTraits::format;
        ...
        if constexpr (tag == decimal_to_binary_rounding::tag_t::to_nearest)
        {
            ...
            if (exponent != 0)
            {
                ...
                // of the same length.
                static_assert(std::is_same_v<format, ieee754_binary32> || std::is_same_v<format, ieee754_binary64>);
                    ...
                    return decltype(interval_type_provider)::invoke_shorter_interval_case(
                        signed_significand_bits, [exponent](auto... additional_args) {
                            return detail::impl<Float, FloatTraits>::template compute_nearest_shorter<
                                return_type, typename decltype(interval_type_provider)::shorter_interval_type,
                                typename policy_holder::trailing_zero_policy, typename policy_holder::binary_to_decimal_rounding_policy,
                                typename policy_holder::cache_policy>(exponent, additional_args...);
                        });
                }
            ...
            return decltype(interval_type_provider)::invoke_normal_interval_case(
                signed_significand_bits, [two_fc, exponent](auto... additional_args) {
                    return detail::impl<Float, FloatTraits>::template compute_nearest_normal<
                        return_type, typename decltype(interval_type_provider)::normal_interval_type,
                        typename policy_holder::trailing_zero_policy, typename policy_holder::binary_to_decimal_rounding_policy,
        ...
        else if constexpr (tag == decimal_to_binary_rounding::tag_t::left_closed_directed)
        {
            ...
            return detail::impl<Float>::template compute_left_closed_directed<return_type, typename policy_holder::trailing_zero_policy,
        ...
        else
        {
            ...
            return detail::impl<Float>::template compute_right_closed_directed<return_type, typename policy_holder::trailing_zero_policy,
                                                                               typename policy_holder::cache_policy>(two_fc, exponent,
                                                                                                                     shorter_interval);
        }
    });
